...
var ASM_CONSTS = {
  3572732: ($0) => { const jsRet = String(eval(UTF8ToString($0))); const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); stringToUTF8(jsRet, strLoc, len); return strLoc; },  
 3572900: ($0, $1) => { const funcName = UTF8ToString($0); const argJson = UTF8ToString($1); const func = globalThis[funcName]; const args = JSON.parse(argJson || '[]') || []; const jsRet = String(func(...args)); const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); stringToUTF8(jsRet, strLoc, len); return strLoc; },  
 3573211: ($0, $1) => { const timeout = Number(UTF8ToString($0)); const funcPtr = $1; setTimeout(()=>{ Module.ccall( 'exec_callback' , 'number' , ["number"] , [funcPtr] ); Module.ccall( 'del_callback' , 'number' , ["number"] , [funcPtr] ); }, timeout); },  
 3573444: ($0, $1, $2) => { console.log('READ1', $0, $1, $2); },  
 3573482: ($0, $1) => { const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); const result = target[property]; if(!result || !['function','object'].includes(typeof result)) { const jsRet = 'OK' + String(result); const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); stringToUTF8(jsRet, strLoc, len); return strLoc; } const jsRet = 'XX'; const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); stringToUTF8(jsRet, strLoc, len); return strLoc; },  
 3573966: ($0, $1) => { const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); const result = target[property]; console.log('READING', {aa: $0, target, property, result}); if(['function','object'].includes(typeof result)) { let index = Module.targets.getId(result); if(!Module.targets.has(result)) { index = Module.targets.add(result); } console.log({index, result}); return index; } return 0; },  
 3574373: ($0, $1, $2) => { console.log('READ2', $0, $1, $2) },  
 3574410: ($0, $1, $2) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); const newValue = $2; console.log('WRITING', {aa: $0, target, property, newValue}); })() },  
 3574598: ($0, $1, $2) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); const funcPtr = $2; console.log('WRITING FUNCTION', {aa: $0, target, property, funcPtr}); target[property] = (...args) => { console.log('CALLING FUNCTION', funcPtr, {targets: Module.targets}); Module.ccall( 'exec_callback' , 'number' , ["number"] , [funcPtr] ); }; })() },  
 3574968: ($0, $1) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); delete target[property]; })() },  
 3575098: ($0, $1) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); target[property] = null; })() },  
 3575228: ($0, $1) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); target[property] = false; })() },  
 3575359: ($0, $1) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); target[property] = true; })() },  
 3575489: ($0, $1, $2) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); target[property] = $2; })() },  
 3575617: ($0, $1, $2) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); target[property] = $2; })() },  
 3575745: ($0, $1, $2) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); const newValue = UTF8ToString($2); target[property] = newValue; })() },  
 3575914: ($0, $1) => { (() =>{ const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); console.log('DELETING', {target, property}); delete target[property]; })() },  
 3576089: ($0, $1) => { const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); console.log('CHECKING', {target, property, result}); return property in target; },  
 3576261: ($0, $1) => { console.log($0, $1) },  
 3576285: ($0, $1, $2) => { const target = Module.targets.get($0) || globalThis; const eventName = UTF8ToString($1); const funcPtr = $2; const options = {}; const callback = () => { Module.ccall( 'exec_callback' , 'number' , ["number"] , [funcPtr] ); }; target.addEventListener(eventName, callback, options); const remover = () => { target.removeEventListener(eventName, callback, options); return $2; }; return Module.callbacks.add(remover); },  
 3576704: ($0) => { const remover = Module.callbacks.get($0); return remover(); },  
 3576768: ($0, $1) => { const target = Module.targets.get($0) || globalThis; const querySelector = UTF8ToString($1); const result = target.querySelector(querySelector); if(!result) { return 0; } let index = Module.targets.getId(result); if(!Module.targets.has(result)) { index = Module.targets.add(result); } console.log({index, result}); return index; },  
 3577101: ($0) => { console.log('CALL1', $0) },  
 3577130: ($0, $1, $2) => { const target = Module.targets.get($0) || globalThis; const method_name = UTF8ToString($1); const argJson = UTF8ToString($2); const args = JSON.parse(argJson || '[]') || []; console.log('CALLING', {aa: $0, target, method_name, args, targets: Module.targets}); const jsRet = String(target[method_name](...args)); const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); stringToUTF8(jsRet, strLoc, len); return strLoc; },  
 3577563: ($0, $1) => { const target = Module.targets.get($0) || globalThis; const property_name = UTF8ToString($1); console.log('GET1', $0, {target, property_name}); target[property_name] = newValueJson; const jsRet = String(target[property_name]); const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); stringToUTF8(jsRet, strLoc, len); return strLoc; },  
 3577911: () => { return Module.targets.add(globalThis); },  
 3577954: ($0, $1, $2) => { console.log('SCAN1', $0, $1, $2); },  
 3577992: ($0) => { const target = Module.targets.get($0) || globalThis; if(typeof target === 'function') { json = JSON.stringify({__targetId:$0}); } else { try{ json = JSON.stringify({__targetId:$0, ...target}); } catch { json = JSON.stringify({__targetId:$0}); } } console.log('SCANNING', {aa: $0, target}); const jsRet = String(json); const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); console.log(jsRet); stringToUTF8(jsRet, strLoc, len); return strLoc; },  
 3578452: ($0) => { console.log('READD1', $0) },  
 3578482: ($0, $1) => { const target = Module.targets.get($0) || globalThis; const property = $1; const result = target[property]; if(!result || !['function','object'].includes(typeof result)) { const jsRet = 'OK' + String(result); const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); stringToUTF8(jsRet, strLoc, len); return strLoc; } const jsRet = 'XX'; const len = lengthBytesUTF8(jsRet) + 1; const strLoc = _malloc(len); stringToUTF8(jsRet, strLoc, len); return strLoc; },  
 3578952: ($0, $1) => { const target = Module.targets.get($0) || globalThis; const property = UTF8ToString($1); const result = target[property]; console.log('READING', {aa: $0, target, property, result}); if(['function','object'].includes(typeof result)) { let index = Module.targets.getId(result); if(!Module.targets.has(result)) { index = Module.targets.add(result); } console.log({index, result}); return index; } return 0; },  
 3579359: ($0, $1, $2) => { console.log('READD2', $0, $1, $2) }
};
