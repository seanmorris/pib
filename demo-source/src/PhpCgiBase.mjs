...
export class PhpCgiBase
{
	...
	async _request(request)
	{
		...
		let vHostEntrypoint = this.entrypoint, vHostPrefix = this.prefix;
		...
		if(typeof rewrite === 'object')
		{
			...
			path = docroot + (docroot[docroot.length-1] === '/' ? '' : '/') + rewrite.path;
		}
		else
		{
			path = docroot + (docroot[docroot.length-1] === '/' ? '' : '/') + rewrite.substr((vHostPrefix || this.prefix).length);
			scriptName = rewrite;
		}

		if(vHostEntrypoint && vHostPrefix)
		{
			// scriptName = vHostPrefix
			// 	+ (vHostPrefix[vHostPrefix.length-1] === '/' ? '' : '/')
			// 	+ vHostEntrypoint.substr(docroot.length + (docroot[docroot.length-1] === '/' ? 0 : 1));
		...
		return new Promise(async accept => {
			...
			if(extension !== 'php')
			{
				...
				// Rewrite to index
				path = docroot + (docroot[docroot.length-1] === '/' ? '' : '/') + 'index.php';
			}
