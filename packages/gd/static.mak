...
packages/gd/php${PHP_VERSION}-gd.so: ${PHPIZE} third_party/php${PHP_VERSION}-gd/config.m4 packages/freetype/libfreetype.so packages/libpng/libpng.so packages/libjpeg/libjpeg.so
	...
	${DOCKER_RUN_IN_EXT_GD} emconfigure ./configure PKG_CONFIG_PATH=${PKG_CONFIG_PATH} --prefix='/src/lib/php${PHP_VERSION}' --with-php-config=/src/lib/php${PHP_VERSION}/bin/php-config --with-jpeg=/src/lib --with-freetype=/src/lib --cache-file=/tmp/config-cache;
