...
// 	assert.equal(returnValue, String(scalarInt));
// 	assert.equal(returnValueIFFE, String(scalarInt));
// 	assert.equal(stdErr, '');
// });
...
// 	assert.equal(returnValue, String(scalarFloat));

// 	const returnValueIFFE = await php.x`(function() { return ${scalarFloat} ; })()`;
// 	assert.equal(returnValueIFFE, String(scalarFloat));
...
test('Can access JS callbacks via php.r function.', async () => {
	...
	// const exitCode = await php.r`<?php var_dump( ${ Thing } );`;
